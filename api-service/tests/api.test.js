...
const testUser = { id: 4, username: 'rafa', role: 'admin' }//cuidado aca
...
describe('API Service', () => {
  ...
  it('✅ GET /menus/:id - debe obtener un menú por ID', async () => {
    const res = await agent.get('/menus/4').set(authHeader)
  ...
  it('✅ PUT /menus/:id - debe actualizar un menú', async () => {
    const res = await agent.put('/menus/4').set(authHeader).send({
  ...
  it('✅ DELETE /menus/:id - debe eliminar un menú', async () => {
    const res = await agent.delete('/menus/4').set(authHeader)
    if (!res.body.message) {
  ...
  it('✅ DELETE /reservations/:id - debe cancelar la reserva', async () => {
    const res = await agent.delete('/reservations/5').set(authHeader)
    if (!res.body.message) {
